...
**Evolução em 24/09/2026:** `IPaymentDispatchService` grava claim antes do HTTP e ID/resultado depois, em transações separadas. Claim não é lease: não expira liberando reenvio. Queda do processo exige conciliação. A migration preserva registros antigos como `NotStarted` e recusa downgrade se houver claims. A criação de recurso externo não confirma pagamento; webhook e homologação externa continuam pendentes.

**Evolução em 25/09/2026:** consulta canônica e diagnóstico somente leitura estão disponíveis, sem aprovar pagamentos. Snapshot validado e observação do provedor não substituem a futura transação de confirmação. ID ausente não será recuperado automaticamente por referência comercial: associação precisa de evidência e auditoria. Não há liberação de claim, retry de POST, migration ou habilitação de compra neste incremento.
