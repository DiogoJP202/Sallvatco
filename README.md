...
O fluxo interno de preparação de pagamento também está disponível: valida dono do pedido, snapshots e reservas e persiste uma única tentativa Sandbox, sem HTTP. O CI testa migration e concorrência em PostgreSQL temporário. A integração dessa preparação ao envio externo e à tela ainda está pendente; a avaliação de Orders está registrada no [ADR-015](docs/DECISIONS.md#adr-015--preparação-local-independente-da-api-de-checkout).

O gateway também possui contrato Orders separado (`CreateOrderAsync`), com total em string, ID externo próprio e validação de resposta. `OrdersEnabled` e `Enabled` (Preferences) são desligados por padrão e não podem ser habilitados juntos. Os testes usam HTTP simulado; ainda não há envio persistido, ID Orders no banco ou webhook. Essa entrega não habilita compras.
