...
O planejamento da Fase 0, a **Fase 1 — Fundação técnica** e a **Fase 2 — Identidade e clientes** estão concluídos. A **Fase 3 — Catálogo, imagens e estoque** está em andamento: já existem produtos, variantes, histórico de slug, estoque auditável, migration, catálogo público em `/perfumes` e CRUD protegido em `/Admin/Produtos`. O Admin também envia, ordena, escolhe a capa e remove imagens; cada arquivo válido é reprocessado em três versões WebP num volume fora do web root e exibido responsivamente no catálogo. O próximo incremento da fase fecha metadados SEO, destaques da home e a experiência de seleção de variante. Os fluxos de e-mail usam caixa de saída local apenas em Development; o provedor real permanece pendente em `PBD-010`. Vínculo de pedidos guest e provisionamento do primeiro Admin dependem das próximas entidades e decisões comerciais. A documentação em [`docs/`](docs/README.md) é a fonte de verdade do desenvolvimento.
